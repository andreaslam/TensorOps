...
class MLP(Model):
    def __init__(self, loss_criterion, num_layers):
        ...
        self.output_nodes = [activation.activation_output for activation in self.model_layers[-1].layer_output]
        self.targets = [Node(0.0, requires_grad=False) for _ in range(len(self.output_nodes))]
    ...
    def calculate_loss(self, output, target):
        ...
        self.output_nodes = [node.set_value(t) for node, t in zip(self.output_nodes, target)]
        return self.loss
...
if __name__ == "__main__":
    ...
    X = [Node(random.uniform(-1,1), requires_grad=False, weight=False) for _ in range(10)]
    y = model(X)
    print(y)
