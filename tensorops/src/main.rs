use ocl::{enums::DeviceInfo, Device, DeviceType, Platform};

fn main() {
    // Collect all GPUs across all platforms
    let mut gpus: Vec<Device> = Platform::list()
        .iter()
        .filter_map(|platform| Device::list(*platform, Some(DeviceType::GPU)).ok())
        .flatten()
        .collect();
    ...
    if let ocl::enums::DeviceInfoResult::MaxComputeUnits(units) =
        device.info(DeviceInfo::MaxComputeUnits).unwrap()
    {
